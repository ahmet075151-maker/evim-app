...
# KÜRESEL HATA RAPORLAYICI (CRASH LOGGER)
def global_exception_handler(exctype, value, tb):
    ...
    try:
        ...
        if not os.path.exists(dl_dir):
            os.makedirs(dl_dir)
        with open(os.path.join(dl_dir, "evim_hata_raporu.txt"), "a", encoding="utf-8") as f:
            f.write(log_text)
    except Exception:
        pass
    ...
    try:
        ...
        try:
            ...
            if angle != 0:
                ...
                rotated_bitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap.getHeight(), matrix, True)
                if rotated_bitmap is not None:
                    bitmap = rotated_bitmap
        except Exception:
            pass
        
        out = FileOutputStream(dest_path)
        success = bitmap.compress(CompressFormat.JPEG, 90, out)
        out.flush()
        out.close()
    ...
    try:
        ...
        for old_p_dir in old_paths:
            if old_p_dir and old_p_dir != new_p_dir and os.path.isdir(old_p_dir):
                try: names = os.listdir(old_p_dir)
                except: continue
                for f in names:
...
def _copy_uri_to_file(uri, dest_path):
    try:
        ...
        if android_api_level() >= 29:
            autoclass('android.os.FileUtils').copy(ins, fos)
        else:
...
class CameraCapture:
    ...
    def run(cls, on_done):
        ...
        try:
            ...
            obj.finish(False, str(e))
        return True

    def start(self):
        if platform != "android":
            self.finish(False, "Kamera yalnızca Android'de çalışır.")
            return
    ...
    def finish(self, success, path_or_err):
        ...
        try:
            self.on_done(success, path_or_err)
        except: pass
...
class EvimApp(App):
    ...
    def open_add_item_dialog(self, edit_id=None, is_box=False):
        ...
        def take_camera_photo(*a):
            def _cam_done(success, path_or_err):
                if success:
                    photo_state["current_file"] = path_or_err
                    update_photo_preview()
                elif path_or_err:
                    self._show_message("Kamera Hatası", path_or_err)

            try:
                CameraCapture.run(_cam_done)
            except Exception as e:
                self._show_message("Hata", f"Kamera başlatılamadı: {str(e)}")
